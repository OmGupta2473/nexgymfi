import { FormEvent, useState } from 'react';
import { PlanTable } from '../components/PlanTable';
import { useArchivePlan, useCreatePlan, usePlans } from '../hooks/usePlans';
import { getErrorMessage } from '../utils/format';

export const PlansPage = () => {
  const plans = usePlans();
  const createPlan = useCreatePlan();
  const archivePlan = useArchivePlan();
  const [error, setError] = useState('');
  const [form, setForm] = useState({
    name: '',
    durationDays: '30',
    price: '',
    description: '',
  });

  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setError('');

    try {
      await createPlan.mutateAsync({
        name: form.name,
        durationDays: Number(form.durationDays),
        price: Number(form.price),
        description: form.description || undefined,
      });
      setForm({ name: '', durationDays: '30', price: '', description: '' });
    } catch (err) {
      setError(getErrorMessage(err));
    }
  };

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold tracking-tight">Plans</h2>
        <p className="mt-1 text-sm text-zinc-500">
          These records come directly from `/plans`.
        </p>
      </div>

      {error ? (
        <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {error}
        </div>
      ) : null}

      <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
        <div className="overflow-x-auto">
          <PlanTable
            plans={plans.data ?? []}
            onArchive={(id) => {
              if (window.confirm('Archive this plan?')) {
                void archivePlan.mutateAsync(id);
              }
            }}
          />
        </div>

        <form
          onSubmit={handleSubmit}
          className="rounded-lg border border-zinc-200 bg-white p-5"
        >
          <h3 className="font-semibold text-zinc-950">New plan</h3>
          <div className="mt-4 grid gap-3">
            <input
              value={form.name}
              onChange={(event) =>
                setForm((current) => ({ ...current, name: event.target.value }))
              }
              placeholder="Plan name"
              className="h-10 rounded-md border border-zinc-300 px-3 text-sm"
              required
            />
            <input
              value={form.durationDays}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  durationDays: event.target.value,
                }))
              }
              type="number"
              min={1}
              placeholder="Duration days"
              className="h-10 rounded-md border border-zinc-300 px-3 text-sm"
              required
            />
            <input
              value={form.price}
              onChange={(event) =>
                setForm((current) => ({ ...current, price: event.target.value }))
              }
              type="number"
              min={1}
              step="0.01"
              placeholder="Price"
              className="h-10 rounded-md border border-zinc-300 px-3 text-sm"
              required
            />
            <textarea
              value={form.description}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  description: event.target.value,
                }))
              }
              placeholder="Description"
              className="min-h-24 rounded-md border border-zinc-300 px-3 py-2 text-sm"
            />
            <button
              type="submit"
              disabled={createPlan.isPending}
              className="h-10 rounded-md bg-zinc-950 px-4 text-sm font-semibold text-white disabled:opacity-60"
            >
              {createPlan.isPending ? 'Saving...' : 'Create plan'}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
};
